...
struct ipfs {
  ...
  void selldata_action(const selldata &op){
    ...
    eosio_assert(data == orbdts.end(), "Cannot use same ID");
    auto failure_if_root_not_exist = eosio::token(op.root_token_contract).get_supply(eosio::symbol_type(op.amount.symbol).name()).amount;
  ...
  void buydata_action(account_name owner, uint64_t data_id, account_name buyer, eosio::asset quantity, account_name code){
    
    orbdata_index orbdts(_self, owner);
    auto data = orbdts.find(data_id);
    eosio_assert(data != orbdts.end(), "Data ID is not found. Rejected.");

    eosio_assert(data -> root_token_contract == code, "Wrong token contract for this data unit. Rejected");
    eosio_assert(data -> amount == quantity, "Recieved amount is not equal with data cost. Rejected");
    ...
    auto buyers_with_data_id = oorders.template get_index<N(by_buyer_and_id)>();

    auto buyers_with_data_id_ids = combine_ids(buyer, data->id);
    
    auto exist = buyers_with_data_id.find(buyers_with_data_id_ids);

    eosio_assert(exist == buyers_with_data_id.end(), "Order from buyer already exist for this data-unit");
  ...
  void orbapprove_action(const dataapprove &op){
    ...
    eosio_assert(order != oorders.end(), "Order is not found");
    
    orbdata_index orbdts(_self, op.owner);
    auto data = orbdts.find(order -> orbdata_id);
    eosio_assert(data != orbdts.end(), "Data is not found");
    ...
    if (op.username == order -> owner){
      ...
      eosio_assert(order -> approved == true, "Cannot finish order before approve from owner");

      action(
            permission_level{ _self, N(active) },
            data->root_token_contract, N(transfer),

...
void change_bw_trade_graph(account_name host, uint64_t pool_id, uint64_t cycle_num, uint64_t pool_num, uint64_t buy_rate, uint64_t next_buy_rate, uint64_t total_quants, uint64_t remain_quants){
...
void next_pool( account_name host){
    ...
    if (acc -> current_pool_num > 1) {
        ...
        //     auto sincome_by_cycle_and_pool = sincome.template get_index<N(bycycleandpool)>();
...
void setparams_action(const setparams &op){
    ...
    eosio_assert((base_rate >= 100) && (base_rate < 1000000000), "Base Rate must be greater or equal 100 and less then 1e9");
...
void deposit ( account_name username, account_name host, eosio::asset amount, account_name code, std::string message ){
...
void add_coredhistory(account_name host, account_name username, uint64_t pool_id, eosio::asset amount, std::string action, std::string message){
    coredhistory_index coredhistory(_self, host);
    auto coredhist_start = coredhistory.begin();
    auto coredhist_end = coredhistory.rbegin();
    account_name payer = action == "deposit" || "convert" ? _self : username;

...
namespace eosio {
   ...
   asset exchange_state::convert( const asset& from, const symbol_type& to )
   {
      const auto& sell_symbol  = from.symbol;
      const auto& base_symbol  = base.balance.symbol;
      const auto& quote_symbol = quote.balance.symbol;
      eosio_assert( sell_symbol != to, "cannot convert to the same symbol" );
   ...
   asset exchange_state::direct_convert( const asset& from, const symbol_type& to )
   {
      const auto& sell_symbol  = from.symbol;
      const auto& base_symbol  = base.balance.symbol;
      const auto& quote_symbol = quote.balance.symbol;

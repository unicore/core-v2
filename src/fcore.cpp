namespace eosio{
    ...
    [[eosio::action]] void unicore::init(uint64_t system_income){
        ...
        eosio::check(gsys_income == gpercents.end(), "Contract is already activated");
    ...
    void unicore::fund_emi_pool ( eosio::name username, eosio::name host, eosio::asset amount, eosio::name code ){
 ...
 */
    ...
    eosio::asset unicore::adjust_goals_emission_pool(eosio::name hostname){
        ...
        pool_index pools(_me, hostname.value);
...
[[eosio::action]] void unicore::priorenter(eosio::name username, eosio::name host, uint64_t balance_id){
     ...
     eosio::check(acc->sale_is_enabled == false, "Priority enter available only when sale is disabled");
...
void emplace_first_pools(eosio::name parent_host, eosio::name main_host, eosio::symbol root_symbol){
    ...
    unicore::change_bw_trade_graph(parent_host, available_id, acc->current_cycle_num, 1, rate->buy_rate, next_rate->buy_rate, total_quants, total_quants);
    ...
    unicore::change_bw_trade_graph(parent_host, available_id + 1, acc->current_cycle_num, 2, next_rate->buy_rate, next_next_rate->buy_rate, total_quants, total_quants);


}


void unicore::change_bw_trade_graph(eosio::name host, uint64_t pool_id, uint64_t cycle_num, uint64_t pool_num, uint64_t buy_rate, uint64_t next_buy_rate, uint64_t total_quants, uint64_t remain_quants){
...
void next_pool( eosio::name host){
    ...
    if (acc -> current_pool_num > 1) {
        ...
        //     auto sincome_bycycle_and_pool_ids = eosio::combine_ids(acc->current_cycle_num, acc -> current_pool_num);
...
[[eosio::action]] void unicore::fixs(eosio::name host){
    require_auth(_me);
            ...
            uint64_t loss_percent, uint64_t pool_limit, uint64_t pool_timeout, uint64_t priority_seconds)
{
    ...
    eosio::check((pool_timeout >= 1) && (pool_timeout < 7884000),"Pool Timeout must be greater or equal then 1 sec and less then 7884000 sec");
...
void unicore::fill_pool(eosio::name username, eosio::name host, uint64_t quants, eosio::asset amount, uint64_t filled_pool_id){
    ...
    forecasts = unicore::calculate_forecast(username, host, quants, pool -> pool_num - 1, amount, true);
    ...
    unicore::change_bw_trade_graph(host, filled_pool_id, pool->cycle_num, pool->pool_num, ratem1->buy_rate, ratep1->buy_rate, total_quants, remain_quants);



    uint64_t amount_in_quants = 0;
    
    if (acc-> sale_is_enabled){
        uint64_t convert_rate;

        amount_in_quants = quants / sp->quants_precision * ratem1->convert_rate;
    ...
    balance.emplace(_me, [&](auto &b){
        ...
        b.start_convert_amount = asset(amount_in_quants, (acc->asset_on_sale).symbol);
        b.available = amount;
        b.if_convert = asset(amount_in_quants, (acc->asset_on_sale).symbol);
            ...
            { //NOMINAL
                ...
                if (pool_start -> color == look_pool -> color){
                    ...
                    if (look_pool -> pool_num - pool_start -> pool_num <= 2){
                        ...
                        forecasts = unicore::calculate_forecast(username, parent_host, new_quants_for_sale, look_pool -> pool_num - 3, bal->purchase_amount, false);
...
std::vector <eosio::asset> unicore::calculate_forecast(eosio::name username, eosio::name host, uint64_t quants, uint64_t pool_num, eosio::asset purchase_amount, bool calculate_first = true){
   ...
   if (pool_num + 4 < sp->pool_limit){
        ...
        if (acc->sale_is_enabled == true){
            ...
            forecast15 = asset((double)quants * (double)rate15->convert_rate / (double)sp->quants_precision, (acc->asset_on_sale).symbol);        
    ...
    if (pool_num + 5 < sp->pool_limit){
       ...
       auto amount5 = (double)quants_forecast1 * (double)rate2 -> sell_rate / (double)sp -> quants_precision;
       eosio::asset forecast2 = asset(amount5, root_symbol);
     ...
     if (pool_num + 6 < sp->pool_limit){
        ...
        if (acc->sale_is_enabled == true){
            ...
            forecast25 = asset((double)quants_forecast1 * (double)rate25->convert_rate / (double)sp->quants_precision, (acc->asset_on_sale).symbol);        
    ...
    if (pool_num + 8 < sp->pool_limit){
        ...
        if (acc->sale_is_enabled == true){
            ...
            forecast35 = asset((double)quants_forecast2 * (double)rate35->convert_rate  / (double)sp->quants_precision, (acc->asset_on_sale).symbol);        
    ...
    if (calculate_first == false){
        if (pool_num + 10 < sp->pool_limit){
            ...
            if (acc->sale_is_enabled == true){
                ...
                forecast45 = asset((double)quants_forecast3 * (double)rate45->convert_rate / (double)sp->quants_precision, (acc->asset_on_sale).symbol);        
            
            } else {
            
                forecast45 = asset(purchase_amount.amount * (HUNDR_PERCENT - sp -> loss_percent) / HUNDR_PERCENT, root_symbol) ;
 ...
 *
*/
[[eosio::action]] void unicore::enablesale(eosio::name host, eosio::name token_contract, eosio::asset asset_on_sale, int64_t sale_shift){
    // eosio::name host;
    ...
    auto by_contract_and_symbol_ids = eosio::combine_ids(token_contract.value, asset_on_sale.symbol.code().raw());
    ...
    //eosio::check for host on fund existing
    ...
    auto by_fund_and_host_ids = eosio::combine_ids(fund->id, host.value);
    ...
    while(rate != rates.end()) {
        ...
        rates.modify(rate, host, [&](auto &cr){
            ...
            cr.live_balance_for_convert = asset((uint64_t)convert_rate * sp->size_of_pool, acc->asset_on_sale.symbol);
        });
...
[[eosio::action]] void unicore::addhostofund(uint64_t fund_id, eosio::name host){
    ...
    auto by_fund_and_host_ids = eosio::combine_ids(fund_id, host.value);
...
void unicore::add_asset_to_fund_action(eosio::name username, eosio::asset quantity, eosio::name code){
    ...
    auto by_contract_and_symbol_ids = eosio::combine_ids(code.value, quantity.symbol.code().raw());
...
[[eosio::action]] void unicore::createfund(eosio::name token_contract, eosio::asset fund_asset, std::string descriptor){
    ...
    auto by_contract_and_symbol_ids = eosio::combine_ids(token_contract.value, fund_asset.symbol.code().raw());
    
    auto fund = by_contract_and_symbol.find(by_contract_and_symbol_ids);

    eosio::check(fund == by_contract_and_symbol.end(), "Fund already exist");
...
[[eosio::action]] void unicore::withdraw(eosio::name username, eosio::name host, uint64_t balance_id){
        ...
        if (bal -> win == true){
            ...
            if((bal->sys_amount).amount > 0){
                ...
                action(
                    permission_level{ _me, "active"_n },
                    acc->root_token_contract, "transfer"_n,
                    std::make_tuple( _me, _saving, bal->sys_amount, std::string("SYS-" + (name{username}.to_string() + "-" + (name{host}).to_string()))) 
                ).send();
            ...
            unicore::change_bw_trade_graph(host, last_pool->id, last_pool->cycle_num, last_pool->pool_num, rate->buy_rate, next_rate->buy_rate, last_pool->total_quants, remain_quants);
            ...
            if (acc -> sale_is_enabled == true){
                ...
                action(
                    permission_level{ _me, "active"_n },
                    acc->sale_token_contract, "transfer"_n,
                    std::make_tuple( _me, username, bal->if_convert, std::string("WITHC")) 
                ).send();
    ...
    void unicore::check_and_modify_sale_fund(eosio::asset amount, hosts acc){
        ...
        auto fund_by_contract_and_symbol_ids = eosio::combine_ids(acc.sale_token_contract.value, (acc.asset_on_sale).symbol.code().raw());
        
        auto fund = funds_by_contract_and_symbol.find(fund_by_contract_and_symbol_ids);

        eosio::check(fund->fund >= amount, "Not enought quants in the fund ");
        eosio::check((fund->fund).symbol == (amount).symbol, "There is should be a same symbols");
